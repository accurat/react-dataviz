...
export default withParentSize(
  class Viz extends React.Component {
    ...
    componentWillUpdate(nextProps) {
      const { margin, parentWidth, parentHeight, flipY } = nextProps
      this.rescale = calculateRescale([0, 0], [parentWidth, parentHeight], margin, {
        flipY,
      })
    }
        ...
        <Context rescale={rescale}>
          <svg width="100%" height="100%" {...svgProps} {...listeners}>
            {debug && (
              <DebugGrid
                rescale={rescale}
                color={typeof debug === 'string' ? debug : 'steelblue'}
              />
            )}
...
function buildListeners(props, mouse, rescale) {
  const listeners = ['onClick', 'onMouseDown', 'onMouseUp', 'onMouseMove']
